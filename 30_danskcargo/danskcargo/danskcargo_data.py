...
class Aircraft(Base):
    ...
    def convert_from_tuple(tuple_):
        try:
            ...
            messagebox.showwarning("", "entires could not be converted to Aircraft")
...
class Transport(Base):
    ...
    id = Column(Integer, primary_key=True)
    date = Column(Date)
    container_id = Column(Integer, ForeignKey("transport.container_id"), nullable=False)
    aircraft_id = Column(Integer, ForeignKey("transport.aircraft_id"), nullable=False)

    def __repr__(self):
        return f"Aircraft({self.id=:4}     {self.date=:5}    {self.container_id=}    {self.aircraft_id=})"
